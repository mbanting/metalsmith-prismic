        ...
        function retrieveContentForFile(fileName, callback, ctx) {
            ...
            if (file.prismic) {
                ...
                for (var queryKey in file.prismic) {
                    ...
                    queryKeys[queryKey] = {"queryString": queryString, "orderings": orderings, "formName": formName};
                }
                ...
                each(Object.keys(queryKeys), function (currentQueryKey, queriedAndProcessedCallback) {
                    ...
                    ctx.api.form(queryKeys[currentQueryKey].formName).query(queryKeys[currentQueryKey].queryString).orderings(queryKeys[currentQueryKey].orderings).ref(ctx.ref).submit(function(err, d) {
                        if (err) {
                            console.error(err);
                        }
                        else {
        ...
        function generateCollectionFiles(fileName, collectionQuery, callback, ctx) {
            if (collectionQuery != null) {
                ...
                for (var i = 0; i < file.prismic[collectionQuery].results.length; i++) {
                    ...
                    newFiles[linkResolver(ctx, file.prismic[collectionQuery].results[i]) + fileSuffix] = newFile;
                }
            }
